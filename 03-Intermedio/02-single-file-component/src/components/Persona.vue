...
export default {
  ...
  created() {
    axios.get('https://randomuser.me/api/').then((respuesta) => {
      // this.persona = respuesta.data.results[0]; // Va a dar error por las reglas de estilo, es algo sencillo,
      // porque debes destructurar o copiar el objeto
      // Con dstructuracion, indico que campos almaceno y en que variables
      // let { name, picture, email } = respuesta.data.results[0];
      // Pero como lo quiero meter todo en un objeto puedo usar
      // Operación de propagación: const y = {...x} O con Object.assign O con JSON.parse(JSON.stringify(obj1));
      // this.persona = Object.assign({}, respuesta.data.results[0]);
